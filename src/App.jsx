  ...
  async function reviewCode() {
    try {
      ...
      const prompt = `
        ...
        3️⃣ Explanation of what the code does.
        4️⃣ Bugs or logical errors.
        5️⃣ Syntax/runtime issues.
      ...
      // ✅ Correct way to get text from API response
      setResponse(result.response.text())
    } catch (error) {
      console.error("Error reviewing code:", error)
      setResponse("⚠️ Error: Could not review code. Check API key or internet.")
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="left h-[87.5%] w-[50%]">
          <div className="tabs mt-5 px-5 mb-3 w-full flex items-center gap-[10px]">
            <Select
              ...
              options={options}
              styles={{
                control: (base) => ({ ...base, backgroundColor: '#18181b', borderColor: '#3f3f46', color: '#fff' }),
                menu: (base) => ({ ...base, backgroundColor: '#18181b', color: '#fff' }),
                singleValue: (base) => ({ ...base, color: '#fff' }),
                option: (base, state) => ({ ...base, backgroundColor: state.isFocused ? '#27272a' : '#18181b', color: '#fff', cursor: 'pointer' }),
              }}
            />
